...
REMOTE_SETTINGS_URL = (
    "https://thigschuch.github.io/SimplyConvertFile/settings/settings.json"
)
# Default check interval for remote settings updates (days)
DEFAULT_CHECK_INTERVAL_DAYS = 7
# Timeout for remote settings fetch (seconds)
...
class SettingsManager:
    ...
    def __init__(self) -> None:
        ...
        self._user_config_dir = Path.home() / ".config" / "simplyconvertfile"
    ...
    def _check_remote_settings(self) -> None:
        ...
        Fetches the latest settings.json from the remote URL and updates
        the local system settings if a newer version is available.
        Checks at most once per week. Fails silently on any error.

        The check respects the user's ``auto_update_settings`` preference
        (defaults to True). User customizations in ``user_settings.json``
        are never modified.
        ...
        try:
            ...
            if not self.get("auto_update_settings", True):
                logger.debug("Remote settings check disabled by user")
                return

            # Check if enough time has passed since last check
            interval_days = self.get(
                "settings_check_interval_days", DEFAULT_CHECK_INTERVAL_DAYS
            )
            interval_seconds = interval_days * 24 * 60 * 60

            timestamp_file = self._user_config_dir / ".last_settings_check"
            if timestamp_file.exists():
                last_check = float(timestamp_file.read_text().strip())
                if time.time() - last_check < interval_seconds:
                    logger.debug("Skipping remote settings check (checked recently)")
                    return

            logger.debug("Checking for remote settings updates...")

            # Fetch remote settings
            req = urllib.request.Request(
                REMOTE_SETTINGS_URL,
                headers={"User-Agent": "SimplyConvertFile"},
            )
            with urllib.request.urlopen(req, timeout=REMOTE_FETCH_TIMEOUT) as response:
                remote_data = json.loads(response.read().decode("utf-8"))

            # Compare versions
            local_settings = self._load_json_file(self._system_config_file)
            if self._needs_version_update(local_settings, remote_data):
                # Write remote settings as new system settings
                with open(self._system_config_file, "w", encoding="utf-8") as f:
                    json.dump(remote_data, f, indent=4, ensure_ascii=False)
                logger.info(
                    "Updated system settings from remote (version {} -> {})",
                    local_settings.get("version", "unknown"),
                    remote_data.get("version", "unknown"),
                )
                # Reload merged settings
                self.load_settings(force_reload=True)
            else:
                logger.debug("System settings are up to date")

            # Update timestamp
            timestamp_file.write_text(str(time.time()))

        except Exception as e:
            logger.debug("Remote settings check failed (non-fatal): {}", str(e))
...
def get_converter_template(
    ...
    if converter_type == "special":
        from simplyconvertfile.utils.validation import FileValidator
